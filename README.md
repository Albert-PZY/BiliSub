...
- B 站扫码登录
- 通过 BV 号或 B 站视频链接解析视频和分 P
- 多分 P 视频先批量选择要获取的分 P，再获取字幕
- 字幕按分 P 流式返回，获取到一个就先显示一个
...
flowchart TB
  ...
  subgraph Server[Next.js 服务端]
    ...
    ResolveApi[app/api/videos/resolve/route.ts\n解析 BV / 分P]
...
app/
  api/
    ...
    videos/resolve/route.ts     解析视频 BV、aid、cid 和分 P
    subtitles/route.ts          按已选分 P 流式获取字幕
  page.tsx                      页面入口
components/                     页面组件
