...
export function QrLogin({
  ...
  const refreshStatus = useCallback(async () => {
    setStatus("checking")
    try {
      const snapshot = await postJson<SessionStatus>("/api/auth/status")
  ...
  const pollLogin = useCallback(
    (session: LoginSession) => {
      ...
      timerRef.current = setInterval(async () => {
        try {
          const result = await postJson<LoginPollResult>("/api/auth/login/poll", {
  ...
  const generateQrCode = useCallback(async () => {
    ...
    setMessage("正在生成二维码")
    try {
      const session = await postJson<LoginSession>("/api/auth/login/start")
